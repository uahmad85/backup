...
all_vlans = ['10.10.6.0/24', '10.10.7.0/24']
pri_zones = ['us-west-2a', 'us-west-2b']
...
associate_vpc_with_zone = route53_session.associate_vpc_with_hosted_zone(HostedZoneId='Z2SNOISYW6SMO7',
                                                                         VPC={'VPCRegion': 'us-west-2',
                                                                              'VPCId': vpc_id})
associate_vpc_with_zone = route53_session.associate_vpc_with_hosted_zone(HostedZoneId='Z5BUHHSE7FRA3',
                                                                         VPC={'VPCRegion': 'us-west-2',
                                                                              'VPCId': vpc_id})
...
pub_sub = mk_subnet(resource_session, vpc_id, False, az=pri_zones[0])
pub_sub1 = mk_subnet(resource_session, vpc_id, False, cidrblk=all_vlans[1], az=pri_zones[1])
pub_sub_id = [pub_sub][0].id
pub_sub1_id = [pub_sub1][0].id

pub_tags = ['public01', 'public02']

sleep(2)
print ResTags(resource_session).tag_subnet(False, pub_sub_id, tags('public01'))
print ResTags(resource_session).tag_subnet(False, pub_sub1_id, tags('public02'))
...
#
#for index in range(len(pri_zones)):
#    xsubnets = mk_subnet(resource_session, vpc_id, False, cidrblk=backend_vlans[index], az=pri_zones[index])
...
rt_id = [table_id][0].id
tag_rt = ResTags(resource_session).tag_routetable(False, rt_id, tags('RouteTable with InternetGateway'))
print tag_rt
...
for x in [pub_sub_id, pub_sub1_id]:
    print associate_with_subnet([table_id][0].id, x)
...
def create_ingress_rules(resource_session, ipprotocol='tcp', sgid=None, uidgp=[], from_port=80, to_port=80,
...
def get_network_ifc_id(resource_session=resource_session, nat_instance_id=None):
...
def get_all_routes(rt_id=None):
    route_list = []
    rt = resource_session.RouteTable(rt_id)
    routes = rt.routes
    for route in routes:
        route_list.append(route.destination_cidr_block)
    return route_list

def_rt_routes = get_all_routes(rt_id=def_rt_id)
backend_rt_routes = get_all_routes(rt_id=backend_rt_id)
peering_rt_routes = get_all_routes(rt_id=rt_id)
...
def_rt_routes.remove('0.0.0.0/0')
backend_rt_routes.remove('0.0.0.0/0')
peering_rt_routes.remove('0.0.0.0/0')

for i in range(len(all_vlans)):
    if not tag_exists(all_vlans[i], def_rt_routes):
        print create_peering_route(cidrblock=all_vlans[i], vpc_p_id=vpc_peering_connection_id)

for i in range(len(all_vlans)):
    if not tag_exists(all_vlans[i], backend_rt_routes):
        print create_peering_route(rt_id=backend_rt_id, cidrblock=all_vlans[i], vpc_p_id=vpc_peering_connection_id)

for subnets in ['172.31.16.0/20', '172.31.32.0/20', '172.31.48.0/20', '172.31.64.0/20']:
    print create_peering_route(rt_id=rt_id, cidrblock=subnets, vpc_p_id=vpc_peering_connection_id)
############################################################################
...
if results.environment == 'development':
    ...
    instance_pri = create_instance(inst_type='t2.micro', subnet_id=get_sub_id_from_tag('public02'),
...
if results.environment == 'staging2':
    ...
    if not tag_exists(api_elb, get_load_balancer()):
        elb = auto_scaling.create_elb(subnets=[get_sub_az('us-west-2a', pri_only_subnets),
                                               get_sub_az('us-west-2b', pri_only_subnets)],
    ...
    if not tag_exists(api_as, get_auto_scaling_groups()):
        scaling_group = auto_scaling.create_auto_scaling(min=1, max=2, lc_name=api_lc, elb_name=api_elb, as_name=api_as,
                                                         ...
                                                         vpcz_id='%s, %s' % (get_sub_az('us-west-2a', pri_only_subnets),
                                                                             get_sub_az('us-west-2b', pri_only_subnets)))
    else:
    ...
    if not tag_exists(images_as, get_auto_scaling_groups()):
        scaling_group = auto_scaling.create_auto_scaling(min=1, max=2, lc_name=images_lc, as_name=images_as,
                                                         ...
                                                         vpcz_id=', '.join([get_sub_az('us-west-2a', pri_only_subnets),
                                                                            get_sub_az('us-west-2b', pri_only_subnets)]))
    else:
