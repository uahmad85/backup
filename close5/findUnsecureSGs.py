        ...
        if 'IpRanges' in sg.ip_permissions[x].keys():
          for i in sg.ip_permissions[x]['IpRanges']:
            if sg.ip_permissions[x]['FromPort'] not in [80, 443, 8080, 123] and i['CidrIp'] == '0.0.0.0/0':
              ...
              jobj = json.dumps(sg.ip_permissions, indent=2)
...
#                         try:
#                                    revoke_ingress(resource,
#                                                       sgid=sg.id,
#                                                       ipp=sg.ip_permissions[x]["IpProtocol"],
#                                                       fp=sg.ip_permissions[x]['FromPort'],
#                                                       tp=sg.ip_permissions[x]['ToPort'])
#                            except ClientError as err:
#                                print err
#
#                            try:
#                                    create_ingress_rules(resource,
#                                                         sgid=sg.id,
#                                                         ipprotocol=sg.ip_permissions[x]["IpProtocol"],
#                                                         from_port=sg.ip_permissions[x]['FromPort'],
#                                                         to_port=sg.ip_permissions[x]['ToPort'],
