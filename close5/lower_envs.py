...
userId = '712818841314'
default_image = 'ami-01f05461'
nat_image = 'ami-030f4133'
default_inst_type = 't2.small'
...
pub_sub02 = mk_subnet(resource_session, vpc_id, dryrun=False, cidrblk='10.100.2.0/24', az='us-west-2b')
pub_sub_id02 = [pub_sub02][0].id

pub_sub03 = mk_subnet(resource_session, vpc_id, dryrun=False, cidrblk='10.100.3.0/24', az='us-west-2c')
...
sleep(1)
ms_sg = create_security_group(vpc_con, group_name='MicroServices', description='for all MicroServices')
ms_sg_id = [ms_sg][0].id
...
print tag_nat_sg, tag_mongodb_sg, tag_elasticsearch_sg

sg_id_list = [ms_sg_id, elasticsearch_sg_id, mongodb_sg_id]
...
for sg in sg_id_list:
    nat_rule = create_ingress_rules(resource_session, sgid=nat_sgid, from_port=80, to_port=80,
                                    ipranges=[], uidgp=[{'GroupId': sg, 'UserId': 'userId'}])

for sg in sg_id_list:
    nat_rule = create_ingress_rules(resource_session, sgid=nat_sgid, from_port=443, to_port=443,
                                    ipranges=[], uidgp=[{'GroupId': sg, 'UserId': 'userId'}])

mongo_rule = create_ingress_rules(resource_session, sgid=mongodb_sg_id, from_port=27017, to_port=27017,
                                  ipranges=[], uidgp=[{'GroupId': ms_sg_id, 'UserId': 'userId'}])

elasticsearch = create_ingress_rules(resource_session, sgid=elasticsearch_sg_id, from_port=9200, to_port=9200,
                                  ipranges=[], uidgp=[{'GroupId': ms_sg_id, 'UserId': 'userId'}])
...
def create_instance(subnet_id, inst_type, resource_session=resource_session, dryrun=False,
    ...
    return instance
nat_instance = create_instance(image_id=nat_image, inst_type='m1.medium',
...
#                              subnet_id=get_sub_id_from_tag('frontend01'),
...
tag_jump_host = ResTags(resource_session).tag_routetable(False, jump_host_id, tags(results.environment + '-jump-host'))
...
if results.environment == 'qa':
    ms_instances = create_instance(inst_type='m4.xlarge', subnet_id=get_sub_id_from_tag('frontend01'),
                                   maxcount=1, instance_sg=ms_sg_id, public_ip=False)
    ...
    try:
        for x in range(len(ms_instances)):
            waiter.wait(DryRun=False, InstanceIds=[ms_instances[x].id])
