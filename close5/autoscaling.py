...
class AutoScalingGroup():
    ...
    def create_elb(self, subnets=default_subnets, listeners_protocol='http',
                   elb_port=80, in_port=80, elb_name=default_tags, security_group=None):
        ...
        response = client.create_load_balancer(
            ...
            Listeners=[{'Protocol': listeners_protocol, 'LoadBalancerPort': elb_port,
                        'InstanceProtocol': self.default_protocol, 'InstancePort': in_port,
                        'SSLCertificateId': 'arn:aws:acm:us-west-2:712818841314:certificate/7e43fc7f-a900-4a2a-b4da-68e1467730cd'}])
        return response
    ...
    def create_auto_scaling(self, min=2, max=10, as_name=default_tags, elb_name=default_tags,
                            lc_name=default_lc_name, vpcz_id=default_vpc_zone_identifier, instance_tags=default_tags):
        client = self.session.client('autoscaling')
        response = client.create_auto_scaling_group(
            ...
            HealthCheckType='ELB',
            DefaultCooldown=10,
            Tags=[{'Key': 'Name', 'Value': instance_tags, 'PropagateAtLaunch': True}])
        return response

    def put_scaling_policy(self, as_name=default_tags, scaling_adjustment=None, policy_name="scaling up"):
        client = self.session.client('autoscaling')
