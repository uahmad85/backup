...
parser.add_argument('-f', '--frontend', action='append', default=[], help='Frontend valns to keep the traffic seprate')
parser.add_argument('-b', '--backend', action='append', default=[], help='Backend valns to keep the traffic seprate')
...
def security_group_exists(vpc_con=None, tag=None):
    for i in vpc_con.security_groups.filter(Filters=[{"Name": "group-name", "Values": [tag]}]):
        if i == "":
            return False
        else:
            print "security group already exists: {0}".format(tag)
            return [i][0].id
...
if tag_exists(environment_name, get_available_vpcs()):
    print "\nVpc already exists '%s'. However, if you want to create a new VPC please use a different " \
          "environment name {dev|staging|qa|lnp} and " % environment_name
    sys.exit()
...
if not tag_exists(environment_name, get_available_vpcs()):
...
vpc_to_peer = create_peering_route(rt_id=nat_table_id, cidrblock='172.31.0.0/16', vpc_p_id=vpc_peering_connection_id)
...
if results.environment == 'staging':
    ...
    api_elb = 'staging-api-elb'
    api_lc = 'staging-api-lc'
    api_as = 'staging-api-as'
    auto_scaling.default_instance_type = 'c3.xlarge'
    ...
    if not security_group_exists(vpc_con=vpc_con, tag=auto_scaling.default_tags):
        core_api_sg = create_security_group(vpc_con, group_name=auto_scaling.default_tags,
                                            description='core-api security group')
        ...
        core_api_sg = security_group_exists(vpc_con=vpc_con, tag=auto_scaling.default_tags)
        core_api_sg_id = [core_api_sg][0].id
    ...
    if not tag_exists(api_lc, get_launch_config()):
        print api_lc
        lunch_config = auto_scaling.create_launch_config(lc_name=api_lc, security_group=[core_api_sg_id])
        print lunch_config
    else:
    ...
    images_elb = 'staging-images-elb'
    images_lc = 'staging-images-lc'
    images_as = 'staging-images-as'
    ...
    if not tag_exists(images_lc, get_launch_config()):
        print images_lc
        lunch_config = auto_scaling.create_launch_config(lc_name=images_lc, security_group=[images_server_sg_id])
        print lunch_config
    else:
    ...
    con_elb = 'staging-con-elb'
    con_lc = 'staging-con-lc'
    con_as = 'staging-con-as'
    ...
    if not tag_exists(con_lc, get_launch_config()):
        print con_lc
        lunch_config = auto_scaling.create_launch_config(lc_name=con_lc, security_group=[consumer_sg_id])
        print lunch_config
    else:
