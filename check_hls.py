...
token_file = '/usr/lib64/nagios/plugins/security/token_of_the_day'
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Critical: error retrieving token from: {}'.format(token_file))
        sys.exit(2)

    try:
        stream = Stream(args.host, args.url, port=args.port,
                        timeout=args.timeout, token=token, ssl=args.ssl)
