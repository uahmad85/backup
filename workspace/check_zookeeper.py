...
class NagiosHandler(object):
    ...
    def analyze(self, opts, cluster_stats):
        ...
        values = ' '.join(values)
        if critical_state:
            print 'ZOOKEEPER CRITICAL: %s = %s ms|%s' % (opts.key, value, values)
            return 2
        
        elif warning_state:
            print 'ZOOKEEPER WARNING: %s = %s ms|%s' % (opts.key, value, values)
            return 1

        else:
            print 'ZOOKEEPER OK: %s = %s ms|%s' % (opts.key, value, values)
            return 0
